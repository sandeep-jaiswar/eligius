    ...
    <div className="flex items-center justify-between rounded-md p-2 hover:bg-gray-100">
      <div className="flex items-center gap-3">
        <div className="relative">
          <Avatar src={avatar || "/placeholder.svg"} alt={name} initials={name[0]} />
          <span
...
export function FriendsList() {
  ...
  const friends: Friend[] = [
    { id: "1", name: "Alex Johnson", avatar: "/placeholder.svg?height=40&width=40", status: "online" },
    { id: "2", name: "Jamie Smith", avatar: "/placeholder.svg?height=40&width=40", status: "offline", lastSeen: new Date(Date.now() - 1000 * 60 * 30) },
    { id: "3", name: "Taylor Wilson", avatar: "/placeholder.svg?height=40&width=40", status: "online" },
    { id: "4", name: "Morgan Lee", avatar: "/placeholder.svg?height=40&width=40", status: "offline", lastSeen: new Date(Date.now() - 1000 * 60 * 60 * 2) }
  ];
        ...
        <Tabs.Content value="all" className="p-0">
          <div className="space-y-1 p-2">
            {filteredFriends.length > 0 ? (
              filteredFriends.map(friend => <FriendListItem key={friend.id} friend={friend} />)
            ) : (
              <div className="py-4 text-center text-gray-500">No friends found</div>
            )}
          </div>
        </Tabs.Content>
        ...
        <Tabs.Content value="online" className="p-0">
          <div className="space-y-1 p-2">
            {filteredFriends.filter(f => f.status === "online").length > 0 ? (
              filteredFriends.filter(f => f.status === "online").map(friend => <FriendListItem key={friend.id} friend={friend} />)
            ) : (
              <div className="py-4 text-center text-gray-500">No online friends</div>
            )}
          </div>
        </Tabs.Content>
